const TOOL_GUIDANCE: Readonly<Record<string, string>> = {
  ...
  list_pipeline_schedules:
    "Use this to survey a project's scheduled pipelines and their `cron`, `next_run_at`, and `active` state, optionally narrowed with `scope` to `active` or `inactive`; use `list_pipelines` for pipelines that already ran and `get_pipeline_schedule` when the schedule ID is known. It is read-only and paginated, requires project access (Maintainer or Owner to see each schedule's `variables` and `inputs`), and returns schedule records or an error when the project is missing or access is denied.",
  ...
  create_pipeline_schedule_variable:
    "Use this to attach a new variable to a pipeline schedule so its scheduled pipelines receive it; use `update_pipeline_schedule_variable` when the key already exists and `create_project_variable` for a project-wide variable. `variable_type` defaults to `env_var` and `key` must match /[a-zA-Z0-9_]+/. It creates remote CI configuration, requires the Maintainer or Owner role, and returns the new variable or a validation, duplicate-key, or permission error.",
