const TOOL_GUIDANCE: Readonly<Record<string, string>> = {
  ...
  list_pipeline_schedules:
    "Use this to survey a project's scheduled pipelines and their `cron`, `next_run_at`, and `active` state, optionally narrowed with `scope` to `active` or `inactive`; use `list_pipelines` for pipelines that already ran and `get_pipeline_schedule` when the schedule ID is known. It is read-only and paginated, requires project access (the Maintainer or Owner role, or ownership of the schedule, to see its `variables` and `inputs`), and returns schedule records or an error when the project is missing or access is denied.",
