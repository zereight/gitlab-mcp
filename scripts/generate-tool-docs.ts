...
function buildIndexPage(groupedToolsList: Array<[ToolsetId, string[]]>): string {
  const lines: string[] = [
    ...
    "- `GITLAB_PERMISSION_MODE=modify` — allows create/update but blocks delete and teardown tools: every `delete_*` tool, the destructive teardown verbs `cancel_pipeline`, `cancel_pipeline_job`, `stop_environment`, `stop_stale_environments`, `unprotect_branch`, plus `push_files` `delete`/`move` actions.",
    "- `GITLAB_READ_ONLY_MODE=true` (deprecated) — same as `readonly`; prefer `GITLAB_PERMISSION_MODE=readonly`.",
    "",
    "The `modify` guard applies to typed tools (`tools/list` and `tools/call`) and to delete-style mutations sent through `execute_graphql`. GraphQL teardown mutations without a delete-style name (for example `pipelineCancel`) are not covered yet — see [PR #755](https://github.com/zereight/gitlab-mcp/pull/755) and [Environment Variables](../configuration/environment-variables.md#gitlab_permission_mode).",
    "",
