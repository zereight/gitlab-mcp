...
No global install? Pin `npx` to the previous stable release and keep the server flags after it, for example `npx -y @zereight/mcp-gitlab@2.1.54 --token=...`. Use `@zereight/mcp-gitlab@latest` if you always want the newest release.
