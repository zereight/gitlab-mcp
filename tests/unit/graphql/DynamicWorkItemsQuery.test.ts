...
describe('DynamicWorkItemsQueryBuilder', () => {
  ...
  beforeEach(() => {
    ...
    mockSchemaIntrospector = {
      getAvailableWidgetTypes: jest.fn().mockReturnValue([
        'ASSIGNEES', 'LABELS', 'MILESTONE', 'DESCRIPTION', 'WEIGHT'
      ]),
  ...
  describe('buildWorkItemsQuery', () => {
    ...
    it('should build query with only requested widgets', () => {
      mockSchemaIntrospector.isWidgetTypeAvailable
        .mockImplementation((widget) => ['ASSIGNEES', 'LABELS'].includes(widget));
    ...
    it('should filter out unavailable widgets', () => {
      mockSchemaIntrospector.isWidgetTypeAvailable
        .mockImplementation((widget) => widget === 'ASSIGNEES');

      mockSchemaIntrospector.getFieldsForType
        .mockReturnValueOnce(mockFieldsAssignees);

      const query = queryBuilder.buildWorkItemsQuery(['ASSIGNEES', 'UNAVAILABLE_WIDGET']);

      expect(mockSchemaIntrospector.isWidgetTypeAvailable).toHaveBeenCalledWith('ASSIGNEES');
      expect(mockSchemaIntrospector.isWidgetTypeAvailable).toHaveBeenCalledWith('UNAVAILABLE_WIDGET');
  ...
  describe('buildWidgetFragments', () => {
    it('should build fragments for available widget types', () => {
      ...
      expect(mockSchemaIntrospector.getFieldsForType).toHaveBeenCalledWith('WorkItemWidgetAssignees');
  ...
  describe('buildSafeFields', () => {
    const buildSafeFields = (queryBuilder: any) =>
      queryBuilder.buildSafeFields.bind(queryBuilder);
    ...
    it('should handle simple scalar widgets', () => {
      const weightFields: FieldInfo[] = [
        { name: 'weight', type: { name: 'Int', kind: 'SCALAR' } }
      ];
  ...
  describe('widget-specific field builders', () => {
    it('should build assignees fields when field is available', () => {
      ...
      expect(mockSchemaIntrospector.hasField).toHaveBeenCalledWith('WorkItemWidgetAssignees', 'assignees');
    ...
    it('should build labels fields when field is available', () => {
      ...
      expect(mockSchemaIntrospector.hasField).toHaveBeenCalledWith('WorkItemWidgetLabels', 'labels');
    ...
    it('should build milestone fields when field is available', () => {
      ...
      expect(mockSchemaIntrospector.hasField).toHaveBeenCalledWith('WorkItemWidgetMilestone', 'milestone');
      expect(result).toHaveLength(8); // milestone block with properties
    });

    it('should build description fields with conditional fields', () => {
      mockSchemaIntrospector.hasField
        .mockImplementation((type, field) => {
          if (field === 'descriptionHtml') return true;
          if (field === 'edited') return false;
          return true;
        });

      const buildDescriptionFields = (queryBuilder as any).buildDescriptionFields.bind(queryBuilder);
