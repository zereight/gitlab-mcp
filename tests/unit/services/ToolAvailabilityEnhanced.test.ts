...
describe('ToolAvailability Enhanced Coverage Tests', () => {
  ...
  describe('Error Handling and Logging', () => {
    it('should handle connection not initialized error with debug logging', () => {
      const error = new Error('Connection not initialized. Call initialize() first.');
      jest.spyOn(ConnectionManager, 'getInstance').mockReturnValue({
        getInstanceInfo: jest.fn().mockImplementation(() => { throw error; })
      } as any);
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        "Tool availability check for 'list_projects': Connection not initialized. Call initialize() first."
      );
    ...
    it('should handle string errors', () => {
      jest.spyOn(ConnectionManager, 'getInstance').mockReturnValue({
        getInstanceInfo: jest.fn().mockImplementation(() => { throw 'String error message'; })
      } as any);
    ...
    it('should handle non-error objects thrown', () => {
      jest.spyOn(ConnectionManager, 'getInstance').mockReturnValue({
        getInstanceInfo: jest.fn().mockImplementation(() => { throw { customError: true, message: 'Custom error' }; })
      } as any);
  ...
  describe('getAvailableTools method', () => {
    ...
    it('should return empty array when connection not initialized', () => {
      jest.spyOn(ConnectionManager, 'getInstance').mockReturnValue({
        getInstanceInfo: jest.fn().mockImplementation(() => { throw new Error('Connection not initialized'); })
      } as any);
  ...
  describe('getUnavailableReason method', () => {
    ...
    it('should return connection not initialized message', () => {
      jest.spyOn(ConnectionManager, 'getInstance').mockReturnValue({
        getInstanceInfo: jest.fn().mockImplementation(() => { throw new Error('Not initialized'); })
      } as any);
  ...
  describe('getToolsByMinVersion method', () => {
    ...
    it('should return tools that require minimum version 15', () => {
      ...
      const allRequirements = tools.map(tool =>
        ToolAvailability.getToolRequirement(tool)?.minVersion || 0
      );
