...
import {
  ListPipelinesSchema,
  GetPipelineSchema,
} from "../../../src/entities/pipelines/schema-readonly";
    ...
    it("should validate source and pipeline parameters with real commit data", async () => {
      ...
      if (result.success) {
        ...
        if (realSha) {
          ...
          console.log(
            `✅ Using real commit SHA from data lifecycle: ${realSha.substring(0, 8)}...`
          );
        }
      ...
      console.log(
        "✅ ListPipelinesSchema validates parameters with real commit data from data lifecycle"
      );
    });
  ...
  describe("GetPipelineSchema", () => {
    ...
    it("should test handler function for single pipeline", async () => {
      ...
      if (paramResult.success) {
        ...
        console.log(
          `✅ GetPipelineSchema handler test successful: ${pipeline.id} (${pipeline.status})`
        );
      }
    });
