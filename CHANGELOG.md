...
- `push_files` optional per-file `action` (`create`/`update`/`delete`/`move`) and `encoding` (`text`/`base64`); `create_or_update_file` optional `encoding`. Defaults keep existing create+text callers working (based on [#513](https://github.com/zereight/gitlab-mcp/pull/513) by [@hodyhq](https://github.com/hodyhq))
