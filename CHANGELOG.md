...
- Opt-in `snippets` toolset: `list_snippets`, `get_snippet`, `create_snippet`, `update_snippet`, and `delete_snippet` for personal and project snippets ([#730](https://github.com/zereight/gitlab-mcp/pull/730))
- Standalone `zereight-mcp-gitlab auth` device-flow command for SSO / headless OAuth ([#591](https://github.com/zereight/gitlab-mcp/issues/591))
- `create_webhook`, `update_webhook`, and `delete_webhook` for project and group hooks ([#713](https://github.com/zereight/gitlab-mcp/issues/713))
- `push_files` optional per-file `action` (`create`/`update`/`delete`/`move`) and `encoding` (`text`/`base64`); `create_or_update_file` optional `encoding`. Existing callers keep `action: create` and the configured `GITLAB_REPO_FILE_ENCODING`. `GITLAB_PERMISSION_MODE=modify` rejects `delete` and `move` (based on [#513](https://github.com/zereight/gitlab-mcp/pull/513) by [@hodyhq](https://github.com/hodyhq))
- Streamable HTTP `GET /mcp` SSE listen stream and `tools.listChanged` capability ([#649](https://github.com/zereight/gitlab-mcp/issues/649), [#651](https://github.com/zereight/gitlab-mcp/issues/651))
...
- Add `list_group_merge_requests` tool for listing merge requests across every project of a group and its subgroups

##### Fixed

- Values interpolated into GitLab URL paths (project/group ids, file paths, branch and tag names, keys, slugs) are now fully percent-decoded and rejected when they contain a `.`/`..` segment, a leading `/`, an empty path segment, or a control character such as NUL ([#760](https://github.com/zereight/gitlab-mcp/pull/760)). Because values are decoded before encoding, a name that literally contains a percent escape (for example a tag named `release%2F1.0`) now resolves to the decoded name (`release/1.0`), and a value mixing a valid escape with a stray `%` is rejected
- `execute_graphql` in `GITLAB_PERMISSION_MODE=modify` now rejects destructive GraphQL mutations whose top-level field names use teardown verbs (for example `environmentStop`, `pipelineCancel`, and `jobUnschedule`), not only delete-style names ([#755](https://github.com/zereight/gitlab-mcp/pull/755))
- Blank list filters (empty strings, whitespace, and empty arrays) are no longer sent as empty query parameters such as `labels[]=` or `iids=`, which made GitLab reject the request with HTTP 400 ([#740](https://github.com/zereight/gitlab-mcp/issues/740))
- `list_issues`, `my_issues`, and `get_issue` descriptions now include issue-management classification hints so agent clients are less likely to bucket them under group or repository tools ([#736](https://github.com/zereight/gitlab-mcp/issues/736))
- `merge_merge_request` no longer strips `sha`, which GitLab 19.2+ groups may require when merging ([#665](https://github.com/zereight/gitlab-mcp/issues/665))
- `list_job_artifacts` no longer fails when GitLab returns `mode: null` on artifact tree entries ([#661](https://github.com/zereight/gitlab-mcp/issues/661))
- `create_tag` no longer fails when GitLab omits `release` from the tag response ([#653](https://github.com/zereight/gitlab-mcp/issues/653))

##### Deprecated

- `GITLAB_READ_ONLY_MODE` and `--read-only=true` — use `GITLAB_PERMISSION_MODE=readonly` or `--permission-mode=readonly` instead (removed in a future major version)

##### Breaking changes

- `update_issue` now returns a slim confirmation by default; pass `full_response=true` for the previous full issue object
- Minimum Node.js version is now 18.17.0 (undici `fetch`)
...
- `update_issue` slim response by default ([#572](https://github.com/zereight/gitlab-mcp/pull/572))

##### Changed

- Replace `node-fetch` with undici `fetch` so installs no longer pull deprecated `node-domexception` ([#712](https://github.com/zereight/gitlab-mcp/issues/712))
...
- **GHSA-cv3r-c5h8-f4g5** / CVE-2026-61560: unauthenticated SSE transport plus `upload_markdown` `file_path` could exfiltrate server credentials ([#554](https://github.com/zereight/gitlab-mcp/pull/554), [#482](https://github.com/zereight/gitlab-mcp/pull/482)). Upgrade to **v2.1.27+** and set `SSE_AUTH_TOKEN` for SSE deployments.
- **GHSA-5648-rgj9-v224**: `execute_graphql` could bypass read-only mode and `GITLAB_ALLOWED_PROJECT_IDS`; Streamable HTTP/SSE could expose server GitLab credentials without MCP auth; garbage tokens could exhaust session slots; CI job traces were returned without untrusted framing ([#571](https://github.com/zereight/gitlab-mcp/pull/571)). Upgrade to **v2.1.30+**.
- **GHSA-4rm9-rfp2-j39q**: `download_release_asset` `direct_asset_path` could escape the release download route and reach other GitLab API paths on the same host, bypassing `GITLAB_ALLOWED_PROJECT_IDS` ([#598](https://github.com/zereight/gitlab-mcp/pull/598)). Upgrade to **v2.1.41+**.
